  ...
  render(){
    return(
      <div>
        ...
        <div>
          <h3>applicants</h3>
          {this.state.applicants.length > 0 ? this.state.applicants.map(user => <div><Link to={`/user/${user.id}`}><p>{user.name}</p></Link><button onClick={() => this.props.actions.addGigToUser(this.props.gig)} className="ui green button">Accept</button><button className="ui red button">Decline</button></div>) : <p>There are currently no applicants for this gig.</p>}
        </div>
        <div>
          <h3>Accepted</h3>
