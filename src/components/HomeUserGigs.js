  ...
  render(){
    return(
      <div className='homeUserGigs'>
        <h3>Upcoming Gigs</h3>
        <ul>
          {this.props.userGigs ? this.props.userGigs.map(gig => <div key={gig.id}><p><Link to={`/gigs/${gig.id}`}>{gig.venue} - {this.parseDate(gig.date)}</Link>{/*<button onClick={() => this.props.gigActions.removeGigFromUser(gig)}>*/}<i onClick={() => this.props.gigActions.removeGigFromUser(gig)} class="trash icon"></i></p></div>): null}
        </ul>
